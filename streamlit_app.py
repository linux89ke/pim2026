...
def render_image_grid() -> None:
    ...
    # We inject CSS that collapses the wrapper to 0-height before rendering the
    # input so the user never sees it or its value.  The <input> DOM node still
    # exists and is found by sendBridge() via its placeholder attribute.
    st.markdown(
        """
        <style>
        /* Hide the bridge text-input completely while keeping the DOM node */
...
# =============================================================================
# SECTION 2 – VALIDATION SUMMARY & EXPANDABLE FLAG RESULTS
# =============================================================================
@st.fragment
def render_validation_summary() -> None:
    """
    Shows:
      • Top-line metrics (total / approved / rejected / flags)
      • One expander per FLAG group listing the affected products
      • Restore button inside each expander row
    Only visible in pre_qc mode with a loaded report.
    """
    if st.session_state.final_report.empty or st.session_state.file_mode == "post_qc":
        return
    ...
    # ── Top metrics ──────────────────────────────────────────────────────────
    st.markdown("---")
    st.header(":material/fact_check: Validation Results", anchor=False)

    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Total Products",  total)
    m2.metric("✅ Approved",     approved,  delta=None)
    m3.metric("❌ Rejected",     rejected,  delta=None)
    approval_pct = round(approved / total * 100, 1) if total else 0
    m4.metric("Approval Rate",  f"{approval_pct}%")

    # ── Progress bar ─────────────────────────────────────────────────────────
    if total:
        st.markdown(
            f"""
            <div style="background:#e0e0e0;border-radius:6px;height:10px;margin:6px 0 18px;">
              <div style="background:{JUMIA_COLORS['success_green']};width:{approval_pct}%;
                          height:10px;border-radius:6px;transition:width .4s;"></div>
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    if rejected == 0:
        st.success("All products passed validation.", icon="✅")
        return

    # ── Per-flag expandable sections ─────────────────────────────────────────
    rej_df = fr[fr["Status"] == "Rejected"].copy()

    # Merge product names / sellers for display
    name_map = (
        data[["PRODUCT_SET_SID", "NAME", "SELLER_NAME", "BRAND", "CATEGORY"]]
        .drop_duplicates("PRODUCT_SET_SID")
        .rename(columns={"PRODUCT_SET_SID": "ProductSetSid"})
        if not data.empty else pd.DataFrame()
    )
    if not name_map.empty:
        rej_df = rej_df.merge(name_map, on="ProductSetSid", how="left")

    # Group by FLAG (fall back to Reason if FLAG empty)
    rej_df["_group"] = rej_df["FLAG"].replace("", pd.NA).fillna(rej_df["Reason"])
    groups = rej_df.groupby("_group", sort=False)

    st.markdown(
        f"<div style='margin-bottom:8px;font-size:13px;color:{JUMIA_COLORS['medium_gray']};'>"
        f"<b>{len(groups)}</b> rejection reason(s) across <b>{rejected}</b> product(s)</div>",
        unsafe_allow_html=True,
    )
    ...
    FLAG_ICONS = {
        ...
        "Duplicate product":    "♊",
        "Wrong Variation":      "🔀",
        "Generic BRAND Issues": "🏷️",
        "Product Warranty":     "📋",
        "Incomplete Smartphone Name": "📱",
    }

    for flag_name, group in groups:
        icon  = FLAG_ICONS.get(str(flag_name), "⚠️")
        count = len(group)
        reason_code = group["Reason"].iloc[0] if "Reason" in group.columns else ""

        with st.expander(f"{icon} {flag_name}  ·  {count} product(s)", expanded=False):
            # Reason code banner
            if reason_code:
                st.markdown(
                    f"<div style='background:{JUMIA_COLORS['light_gray']};padding:8px 12px;"
                    f"border-left:4px solid {JUMIA_COLORS['primary_orange']};"
                    f"border-radius:4px;font-size:12px;margin-bottom:12px;'>"
                    f"<b>Rejection Code:</b> {reason_code}</div>",
                    unsafe_allow_html=True,
                )

            # Table header
            cols = st.columns([3, 2, 2, 1.5, 1])
            cols[0].markdown("**Product Name**")
            cols[1].markdown("**Brand**")
            cols[2].markdown("**Seller**")
            cols[3].markdown("**SID**")
            cols[4].markdown("**Action**")
            st.markdown(
                f"<hr style='margin:4px 0 8px;border-color:{JUMIA_COLORS['border_gray']};'>",
                unsafe_allow_html=True,
            )

            for _, row in group.iterrows():
                sid    = str(row["ProductSetSid"])
                ...
                c1.markdown(
                    f"<span title='{name}' style='font-size:12px;'>{name}</span>",
                    unsafe_allow_html=True,
                )
                c2.markdown(
                    f"<span style='font-size:12px;color:{JUMIA_COLORS['primary_orange']};'>{brand}</span>",
                    unsafe_allow_html=True,
                )
                c3.markdown(
                    f"<span style='font-size:11px;color:{JUMIA_COLORS['medium_gray']};'>{seller}</span>",
                    unsafe_allow_html=True,
                )
                c4.markdown(
                    f"<span style='font-size:10px;color:#aaa;'>{sid[:12]}…</span>",
                    unsafe_allow_html=True,
                )
                with c5:
                    if st.button("↩ Restore", key=f"val_restore_{sid}",
                                 help="Move back to Approved"):
            ...
            # Bulk restore for this flag group
            st.markdown("")
            all_sids_in_group = group["ProductSetSid"].tolist()
            if st.button(
                f"↩ Restore all {count} '{flag_name}' rejections",
                key=f"val_restore_all_{flag_name}",
                type="secondary",
            ):
                for sid in all_sids_in_group:
                    restore_single_item(sid)
                try:
                    st.rerun(scope="fragment")
                except TypeError:
...
# =============================================================================
# SECTION 3 – IMAGE & CATEGORY REVIEW GRID
# =============================================================================
render_image_grid()
