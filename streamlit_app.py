...
if 'pending_grid_rejections' not in st.session_state: st.session_state.pending_grid_rejections = {}
...
# --- Flush pending grid rejections before results section renders ---
if st.session_state.get('pending_grid_rejections'):
    _pending = st.session_state.pending_grid_rejections
    st.session_state.pending_grid_rejections = {}
    _rgroups: dict = {}
    for _sid, _rkey in _pending.items():
    ...
    for _rkey, _sids in _rgroups.items():
        ...
        _total_flushed += len(_sids)
    st.session_state.exports_cache.clear()
    st.session_state.display_df_cache.clear()
    st.session_state.main_toasts.append((f"Rejected {_total_flushed} product(s)", "✅"))
...
def render_image_grid():
    ...
    # Only needs to handle 'reject' now — no selection_update
    components.html("""
    <script>
    (function() {
      ...
      target.addEventListener("message", function(e) {
        ...
        if (action !== "reject") return;   // only care about rejections now
        // Find the hidden bridge input by aria-label
    ...
    # ── Process ONLY reject messages from iframe ──────────────────────────────
        ...
        try:
            ...
            if action == "reject":
                ...
                if isinstance(payload, dict) and payload:
                    # Hand off to main flow — it flushes before results render
                    st.session_state.pending_grid_rejections.update(payload)
